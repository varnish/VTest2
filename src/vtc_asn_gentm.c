...
/* UPDATE: OpenSSL does now (since 2017) support converting an
 * ASN1_GENERALIZEDTIME to a struct tm via ASN1_TIME_to_tm().
 *
 * Our distros Xenial, Stretch and CentOS7 predate this.
 *
 * Kill this file once we're done supporting those.
 */
...
int VTC_ASN1_TIME_to_tm(const ASN1_GENERALIZEDTIME *d, struct tm *tm);
/*
...
VTC_ASN1_TIME_to_tm(const ASN1_GENERALIZEDTIME *d, struct tm *tm) {
	...
	return (asn1_generalizedtime_to_tm(tm, d));
}
